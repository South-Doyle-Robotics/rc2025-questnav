...
public class QuestNav {
    ...
    private Rotation2d headingError = new Rotation2d();
    private Translation2d positionError = new Translation2d();
    ...
    public void setPose(Pose2d pose) {
        questMosi.set(1);
        headingError = pose.getRotation();
        positionError = pose.getTranslation();
    }
    ...
    private Translation2d getHMDTranslation() {
        ...
        /* was: new Translation2d(translation[2], -translation[0])
        but quest is rotated 90 deg CCW so we correct it here */
        Translation2d pos = new Translation2d(translation[0], translation[2]);

        /* apply zero position, then rotate it around (0,0) to account for quest heading error
        (if you start questnav with the quest in a funny direction, this is required) */
        pos = pos.plus(positionError).rotateAround(new Translation2d(), headingError);
    ...
    private Rotation2d getHMDRotation() {
        ...
        float rotation = (angles[1] + (float)headingError.getDegrees()) % 360.0f;
        if (rotation < 0) {
            rotation += 360.0f;
        }
